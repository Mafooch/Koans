...
class AboutRegularExpressions < Neo::Koan
  ...
  # character class is for matching one of the characters whereas alternation
  # can be used to match a group of characters together

  def test_scan_is_like_find_all
    assert_equal(["one", "two", "three"], "one two-three".scan(/\w+/))
  end

  # finding first occurrence of 't' followed by 0 or more word characters, 'two'
  # in this case which is saved to $1 variable. we replace this occurrence with
  # range of 0-1 in that variable, which is 't' so 'two' becomes simply 't'

  def test_sub_is_like_find_and_replace
    assert_equal("one t-three", "one two-three".sub(/(t\w*)/) { $1[0, 1] })
  end

  # same as above except finds all occurences
